...
def wrn_50_2_features(in_chs=3, activation_fn=nn.ReLU()):
    features = nn.Sequential(  # Sequential,
        nn.Conv2d(in_chs, 64, (7, 7), (2, 2), (3, 3), 1, 1, bias=False),
        nn.BatchNorm2d(64),
        activation_fn,
            ...
            nn.Sequential(  # Sequential,
                LambdaMap(lambda x: x,  # ConcatTable,
                          nn.Sequential(  # Sequential,
                              ...
                              nn.BatchNorm2d(1024),
                              activation_fn,
                              nn.Conv2d(1024, 1024, (3, 3), (1, 1), (1, 1), 1, 1, bias=False),  # 2,2
                              nn.BatchNorm2d(1024),
                              activation_fn,
                          ...
                          nn.Sequential(  # Sequential,
                              nn.Conv2d(1024, 2048, (1, 1), (1, 1), (0, 0), 1, 1, bias=False),  # 2,2
                              nn.BatchNorm2d(2048),
                          ),
                          ),
                LambdaReduce(lambda x, y: x + y),  # CAddTable,
...
class Wrn50_2(nn.Module):
    def __init__(self, in_chs=3, num_classes=1000, activation_fn=nn.ReLU(), drop_rate=0., global_pool='avg'):
        super(Wrn50_2, self).__init__()
        self.drop_rate = drop_rate
        self.num_classes = num_classes
        self.num_features = 2048
        self.global_pool = global_pool
        self.features = wrn_50_2_features(in_chs=in_chs, activation_fn=activation_fn)
...
def wrn50_2(pretrained=False, in_chs=3, num_classes=1000, **kwargs):
    model = Wrn50_2(in_chs=in_chs, num_classes=num_classes, **kwargs)
    if pretrained:
