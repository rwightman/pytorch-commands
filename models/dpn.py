...
def dpn68(in_chs=3, num_classes=1000, pretrained=False, test_time_pool=7):
    model = DPN(
        small=True, in_chs=in_chs, num_init_features=10, k_r=128, groups=32,
...
def dpn68b(in_chs=3, num_classes=1000, pretrained=False, test_time_pool=7):
    model = DPN(
        small=True, in_chs=in_chs, num_init_features=10, k_r=128, groups=32,
...
def dpn92(in_chs=3, num_classes=1000, pretrained=False, test_time_pool=7, extra=True):
    model = DPN(
        in_chs=in_chs, num_init_features=64, k_r=96, groups=32,
...
def dpn98(in_chs=3, num_classes=1000, pretrained=False, test_time_pool=7):
    model = DPN(
        in_chs=in_chs, num_init_features=96, k_r=160, groups=40,
...
def dpn131(in_chs=3, num_classes=1000, pretrained=False, test_time_pool=7):
    model = DPN(
        in_chs=in_chs, num_init_features=128, k_r=160, groups=40,
...
def dpn107(in_chs=3, num_classes=1000, pretrained=False, test_time_pool=7):
    model = DPN(
        in_chs=in_chs, num_init_features=128, k_r=200, groups=50,
...
class InputBlock(nn.Module):
    def __init__(self, num_init_features, in_chs=3, kernel_size=7,
                 padding=3, activation_fn=nn.ReLU(inplace=True)):
        super(InputBlock, self).__init__()
        self.conv = nn.Conv2d(
            in_chs, num_init_features, kernel_size=kernel_size, stride=2, padding=padding, bias=False)
...
class DPN(nn.Module):
    def __init__(self, small=False, in_chs=3, num_init_features=64, k_r=96, groups=32,
        ...
        # conv1
        if small:
            blocks['conv1_1'] = InputBlock(num_init_features, in_chs=in_chs, kernel_size=3, padding=1)
        else:
            blocks['conv1_1'] = InputBlock(num_init_features, in_chs=in_chs, kernel_size=7, padding=3)
        ...
        inc = inc_sec[3]
        r = (k_r * bw) // (64 * bw_factor)
        blocks['conv5_1'] = DualPathBlock(in_chs, r, r, bw, inc, groups, 'down2', b)
        in_chs = bw + 3 * inc
